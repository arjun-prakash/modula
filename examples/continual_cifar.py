...
def sample_task_class_pairs(key, num_tasks, num_classes=10):
    if num_tasks <= 0:
    ...
    for _ in range(num_tasks):
        ...
        pairs.append((int(classes[0]), int(classes[1])))
    return pairs
...
def plot_task_accuracies(results: Dict[str, List[Dict[str, float]]], plots_dir: Path) -> None:
    ...
    ax_train.set_title("Continual CIFAR train accuracy per task")
    ax_test.set_title("Continual CIFAR test accuracy per task")
    ...
    fig.tight_layout()
    fig.savefig(plots_dir / "continual_cifar_accuracy.png", dpi=300)
    plt.close(fig)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Continual learning on binary CIFAR-10 tasks")
    parser.add_argument("--num-tasks", type=int, default=45, help="Number of sequential tasks")
    parser.add_argument("--steps-per-task", type=int, default=1000, help="Training steps allocated to each task")
    parser.add_argument("--batch-size", type=int, default=128, help="Mini-batch size")
    parser.add_argument("--learning-rate", type=float, default=5e-2, help="Learning rate for all methods")
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Seed for sampling class pairs (defaults to seed+1)",
    )
    parser.add_argument(
        "--methods",
        type=str,
    ...
    parser.add_argument(
        "--results-path",
        type=Path,
        default=Path("results/continual_cifar_results.json"),
