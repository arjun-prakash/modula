...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(description="Multiclass CIFAR-100 benchmark with Muon-style manifold methods")
    add_common_arguments(
        parser,
        dataset_name="cifar100",
        default_learning_rates=[5e-2, 1e-2, 5e-3, 1e-3],
        default_steps=3000,
        default_batch_size=128,
        default_eval_every=150,
        default_results_path=Path("results/benchmark_cifar100_results.json"),
        default_plots_dir=Path("plots") / "benchmark_cifar100",
    )
...
def main(argv=None):
    ...
    for method_idx, method in enumerate(args.methods):
        ...
        for lr_idx, learning_rate in enumerate(args.learning_rates):
            ...
            run_name = f"cifar100-{method}-lr{learning_rate:.3g}"
