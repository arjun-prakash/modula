...
def main(argv=None):
    ...
    for method_idx, method in enumerate(args.methods):
        ...
        for hidden_idx, hidden_size in enumerate(args.hidden_sizes):
            ...
            trunk, head = build_cifar_mlp_models(dataset.num_classes, hidden_size, trunk=args.trunk)
            for lr_idx, learning_rate in enumerate(args.learning_rates):
                ...
                run_name = f"cifar10-mlp-{args.trunk}-{method}-h{hidden_size}-lr{learning_rate:.3g}"
                logger = create_run_logger(
                    ...
                    tags=["cifar10", "mlp", args.trunk, method, f"hidden_size={hidden_size}"],
                )
                try:
